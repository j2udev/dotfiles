...
dockerBuild() {
  ...
  if "$DFM_DOCKER_NO_CACHE" ; then
    docker build --no-cache --target "$DFM_DOCKER_BUILD_STAGE" -t "$DFM_DOCKER_IMAGE_NAME" -f "$DFM_DIR"/"$DFM_DOCKERFILE" "$DFM_DIR"
  else
    docker build --target "$DFM_DOCKER_BUILD_STAGE" -t "$DFM_DOCKER_IMAGE_NAME" -f "$DFM_DIR"/"$DFM_DOCKERFILE" "$DFM_DIR"
  fi
}
